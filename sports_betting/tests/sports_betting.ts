...
describe("SportsBetting", () => {
  // Configure the client to use the local cluster.
  anchor.setProvider(anchor.AnchorProvider.env());
  const program = anchor.workspace
    .SportsBetting as Program<SportsBetting>;
  ...
  it("Is initialized!", async () => {
    ...
    console.log("Initializing reveal winner computation definition");
    const initRWSig = await initRevealWinnerCompDef(program, owner, false);
    console.log(
      "Reveal winner computation definition initialized with signature",
      initRWSig
    );
    ...
    const revealWinnerEventPromise = awaitEvent("revealWinnerEvent");
    ...
    const revealWinnerEvent = await revealWinnerEventPromise;
    expect(revealWinnerEvent.winner).to.equal(owner.publicKey);
  });

  async function initRevealWinnerCompDef(
    program: Program<SportsBetting>,
    ...
    console.log("Init reveal winner computation definition transaction", sig);
