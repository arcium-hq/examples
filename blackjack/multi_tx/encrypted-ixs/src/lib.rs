...
mod circuits {
    ...
    pub fn shuffle_and_deal_cards(
        ...
        Enc<Mxe, Deck>,    // 16 + 32 x 52 = 1680
        Enc<Mxe, Hand>,    // 16 + 32 x 11 = 368
        Enc<Shared, Hand>, // 16 + 32 x 11 + 32 = 400
        Enc<Shared, u8>,   // 16 + 32 x 1 + 32 = 80
    ) {
        let mut initial_deck = INITIAL_DECK;
        ArcisRNG::shuffle(&mut initial_deck);

        let deck = mxe.from_arcis(Deck(initial_deck));

        let mut dealer_cards = [53; 11];
        dealer_cards[0] = initial_deck[1];
        dealer_cards[1] = initial_deck[3];

        let dealer_hand = mxe_again.from_arcis(Hand(dealer_cards));

        let mut player_cards = [53; 11];
        player_cards[0] = initial_deck[0];
        player_cards[1] = initial_deck[2];

        let player_hand: Enc<Shared, Hand> = client.from_arcis(Hand(player_cards));
