...
declare_id!("9ihCYfyqBiasBKNHA1ymLpBWr399Z8nXA4eG3XCPiQWM");
...
pub mod blackjack {
    ...
    pub fn initialize_blackjack_game(
        ...
        ctx.accounts.temp_data_storage.load_init()?;
    ...
    pub fn shuffle_and_deal_cards_callback(
        ...
        output: ComputationOutputs<ShortVec<u8>>,
    ) -> Result<()> {
        let o = match output {
        ...
        msg!("Current length passed: {}", o.data.len());

        let temp_data_storage = &mut ctx.accounts.temp_data_storage.load_mut()?;

        let start = temp_data_storage.length as usize;
        temp_data_storage.length += o.data.len() as u32;
        temp_data_storage.data[start..start + o.data.len()].copy_from_slice(&o.data);
    ...
    pub fn finally_init_game(
        ...
        blackjack_game.deck_nonce =
            *bytemuck::from_bytes::<u128>(&temp_data_storage.data[offset..offset + 16]);
        offset += 16;
        blackjack_game.deck = *bytemuck::from_bytes::<[[u8; 32]; 52]>(
            &temp_data_storage.data[offset..offset + 52 * 32],
        );
        offset += 32 * 52;
        blackjack_game.dealer_nonce =
            *bytemuck::from_bytes::<u128>(&temp_data_storage.data[offset..offset + 16]);
        offset += 16;
        blackjack_game.dealer_hand = *bytemuck::from_bytes::<[[u8; 32]; 11]>(
            &temp_data_storage.data[offset..offset + 32 * 11],
        );
        offset += 32;
        blackjack_game.player_enc_pubkey =
            *bytemuck::from_bytes::<[u8; 32]>(&temp_data_storage.data[offset..offset + 32]);
        offset += 32;
        blackjack_game.client_nonce =
            *bytemuck::from_bytes::<u128>(&temp_data_storage.data[offset..offset + 16]);
        offset += 16;
        blackjack_game.player_hand = *bytemuck::from_bytes::<[[u8; 32]; 11]>(
            &temp_data_storage.data[offset..offset + 32 * 11],
        );
        offset += 32;
        let dealer_client_pubkey =
            *bytemuck::from_bytes::<[u8; 32]>(&temp_data_storage.data[offset..offset + 32]);
        offset += 32;
        let dealer_client_nonce =
            *bytemuck::from_bytes::<u128>(&temp_data_storage.data[offset..offset + 16]);
        offset += 16;
...
pub struct InitializeBlackjackGame<'info> {
    ...
    #[account(
        ...
        payer = payer,
        space = 8 + 2592,
        seeds = [b"temp_data_storage".as_ref(), game_id.to_le_bytes().as_ref()],
        bump,
    )]
    pub temp_data_storage: AccountLoader<'info, TempDataStorage>,
}
...
#[derive(Accounts)]
pub struct FinallyInitGame<'info> {
    pub arcium_program: Program<'info, Arcium>,
    #[account(
        address = derive_comp_def_pda!(COMP_DEF_OFFSET_SHUFFLE_AND_DEAL_CARDS)
    )]
...
/// and game progress. The deck is stored as 52 32-byte encrypted chunks that together
