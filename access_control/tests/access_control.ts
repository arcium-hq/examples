...
describe("AccessControl", () => {
  ...
  it("Is initialized!", async () => {
    ...
    console.log(
      "Initializing share answer to ultimate question of life computation definition"
    );
    const initATSig = await initShareAnswerToUltimateQuestionOfLifeCompDef(
      program,
      owner,
      false
    );
    console.log(
      "Share answer to ultimate question of life computation definition initialized with signature",
      initATSig
    );
    ...
    const plaintext = [val1];

    const nonce = randomBytes(16);
    const ciphertext = cipher.encrypt(plaintext, nonce);

    const answerEventPromise = awaitEvent("answerEvent");

    // TODO: Remove this sleep once the CI bug is solved
    await new Promise((resolve) => setTimeout(resolve, 100));

    const queueSig = await program.methods
      .shareAnswerToUltimateQuestionOfLife(
      ...
      .accountsPartial({
        ...
        compDefAccount: getCompDefAcc(
          program.programId,
          Buffer.from(
            getCompDefAccOffset("share_answer_to_ultimate_question_of_life")
          ).readUInt32LE()
        ),
      })
    ...
    const answerEvent = await answerEventPromise;
    const decrypted = cipher.decrypt(
      [answerEvent.answer],
      answerEvent.nonce
    )[0];
    expect(decrypted).to.equal(val1);
  });

  async function initShareAnswerToUltimateQuestionOfLifeCompDef(
    ...
    const offset = getCompDefAccOffset(
      "share_answer_to_ultimate_question_of_life"
    );
    ...
    const sig = await program.methods
      .initShareAnswerToUltimateQuestionOfLifeCompDef()
      .accounts({
    ...
    console.log(
      "Init share answer to ultimate question of life computation definition transaction",
      sig
    );

    if (uploadRawCircuit) {
      const rawCircuit = fs.readFileSync(
        "build/share_answer_to_ultimate_question_of_life.ts"
      );
